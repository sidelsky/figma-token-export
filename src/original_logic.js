    ...
    // DOM elements
    let exportBtn, downloadBtn, status, previewSection, previewContent, previewTitle;
    let tokenSelection, tokenCollections, selectAllToggle;
    ...
    function showStatus(type, message) {
      ...
      status.className = 'mt-4 p-3 rounded-md text-sm border';
    ...
    function convertToReactNative(data) {
      let rn = '// Design Tokens exported from Figma\n';
      rn += '// Exported at: ' + data.metadata.exportedAt + '\n';
      rn += '// File: ' + (data.metadata.figmaFileKey || 'Unknown') + '\n';
      rn += '// Collections: ' + Object.keys(data.collections).join(', ') + '\n\n';

      rn += 'import { StyleSheet } from \'react-native\';\n\n';

      rn += '// Design Tokens\n';
      rn += 'export const tokens = {\n';
      ...
      for (const collectionName in data.collections) {
        ...
        for (const varName in collection.variables) {
          ...
          for (const modeName in variable.values) {
            const value = variable.values[modeName];
            const tokenName = toCamelCase(varName);
            const comment = variable.description ? ' // ' + variable.description : '';

            // Format value for React Native
            let rnValue = value;
            if (variable.type === 'FLOAT') {
              rnValue = parseFloat(value);
            } else if (typeof value === 'string') {
              rnValue = '\'' + value + '\'';
            }

            rn += '    ' + tokenName + ': ' + rnValue + ',' + comment + '\n';
            break; // Use first mode only
          }
        }
      ...
      // Create StyleSheet styles
      rn += '// Utility Styles\n';
      rn += 'export const styles = StyleSheet.create({\n';

      // Add some common utility styles
      rn += '  // Typography Styles\n';
      rn += '  text: {\n';
      rn += '    fontFamily: \'System\',\n';
      rn += '  },\n\n';
      ...
      rn += '  // Add your custom styles here using tokens\n';
      rn += '  // Example:\n';
      rn += '  // primaryButton: {\n';
      rn += '  //   backgroundColor: tokens.colors.primary,\n';
      rn += '  //   paddingHorizontal: tokens.spacing.medium,\n';
      rn += '  //   paddingVertical: tokens.spacing.small,\n';
      rn += '  // },\n';
      ...
      // Usage examples
      rn += '// Usage Examples:\n';
      rn += '// import { tokens, styles } from \'./tokens\';\n\n';
      rn += '// <View style={[styles.container, { backgroundColor: tokens.colors.background }]}>\n';
      rn += '//   <Text style={[styles.text, { color: tokens.colors.primary }]}>Hello World</Text>\n';
      rn += '// </View>\n';
    ...
    function generateAccessibilityRecommendations(analysis) {
      ...
      if (analysis.summary.criticalIssues > 0) {
        recommendations.push('🚨 Critical: Fix color contrast issues that fail WCAG standards');
      }

      if (analysis.colorContrast.length > 0) {
        recommendations.push('🎨 Review color combinations for better contrast ratios');
      }

      if (analysis.typography.length > 0) {
        recommendations.push('📝 Consider larger font sizes for improved readability');
      }
      ...
      if (analysis.summary.totalIssues === 0) {
        recommendations.push('✅ Great! No accessibility issues detected');
      }
